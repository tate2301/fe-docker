...
import config from '../../Consonant/__tests__/mocks/consonant.json';

import makeInit from './utils/init';
...
const init = makeInit(ConsonantWrapper, config);
...
describe('Consonant/FilterItem', () => {
    ...
    test('should render with header', async () => {
        init({ header: { enabled: true } });

        // search for header element in whole DOM tree
        const headerElement = screen.queryByTestId('consonant-header');
    ...
    describe('Interaction with UI', () => {
        ...
        test('shouldn`t bookmark card', async () => {
            init({ bookmarks: { bookmarkOnlyCollection: true }, collection: { cardsStyle: '3:2' } });

            // Need wait for api response and state updating
            await waitFor(() => screen.getByTestId('consonant-collection'));

            const [saveBookmarkButton] = screen.queryAllByTestId('consonant-card__bookmark-button');

            expect(saveBookmarkButton).toHaveClass('consonant-card--footer-btn_disabled');
        });
