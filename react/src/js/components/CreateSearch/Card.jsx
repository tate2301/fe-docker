...
export default class Card extends React.Component {
    render() {
        return (
            <div className="create-search__create-card">
                <div
                    className="create-search__create-card-img"
                    style={{ backgroundSize: 'cover', backgroundImage: `url(${this.props.backgroundImage})` }}>
                    <a
                        className="create-search__create-card-hero"
                        href={this.props.url} />
                </div>
                <div className="create-search__create-card-info">
                    <div className="create-search__create-card-tag-label">
                        {this.props.categories.split(' / ').map(category => (
                            <a key={category} href={this.props.url}>{category}</a>
                        ))}
                    </div>
                    <h2 className="create-search__create-card-title">
